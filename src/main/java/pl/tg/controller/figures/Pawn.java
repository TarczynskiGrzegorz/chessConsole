...
public class Pawn extends Figure {
    ...
    private boolean isPossibleMoveForward(FigureColors color, Point startPostition, Point finalPosition, Figure[][] patchwork) {
        System.out.println(startPostition.x + " " + startPostition.y);

        if (color.equals(FigureColors.WHITE)) {
            boolean nextFree = Objects.isNull(patchwork[finalPosition.x][finalPosition.y]) && startPostition.x + 1 == finalPosition.x;
            boolean jump = (isFirstMove() && startPostition.x + 2 == finalPosition.x);
            if (nextFree || jump) {
                return true;
            } else {
                return false;
